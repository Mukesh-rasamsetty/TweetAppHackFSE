...
export const getFollowingPosts = createAsyncThunk(
  "http://localhost:8090/api/v1.0/tweets/all",
  async (thunkAPI) => {
...
async function insertComment(postId, commentContent) {
  console.log('comment contete here-------',commentContent);
  const response = await axios({
    method: "post",
    url: "http://localhost:8090/api/v1.0/tweets/"+localStorage.getItem("userName")+"/reply/"+postId,
   // headers: {
    //  Authorization: "Bearer "+localStorage.getItem("token"),
    //},
    data: {
...
async function updateLove(postId, currentUserId) {
    ...
    const response = await axios({
        method: "put",
        url: "http://localhost:8090/api/v1.0/tweets/"+localStorage.getItem("userName")+"/like/"+postId,
        //headers: {
         //Authorization: "Bearer "+localStorage.getItem("token"),
        //},
...
export const PostSlice = createSlice({
  ...
  reducers: {
      ...
      addComment: (state, action) => {
        console.log('-----------add comment called----',action.payload);
        if (state.followingPosts !== null) {
          for (let i = 0; i < state.followingPosts.length; i++) {
            if (state.followingPosts[i].id === action.payload.postId) {
