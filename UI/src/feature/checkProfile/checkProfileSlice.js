...
async function deletePost(postIds) {
  console.log("in delete");
    const response = await axios({
      method: "delete",
      url: "http://localhost:8090/api/v1.0/tweets/"+localStorage.getItem("userName")+"/delete/"+postIds,
     // headers: {
...
export const getProfilePosts = createAsyncThunk(
  "http://localhost:8090/api/v1.0/tweets/"+localStorage.getItem("userName"),
  async (thunkAPI) => {
   
    const response = await axios({
      method: "get",
      url: "http://localhost:8090/api/v1.0/tweets/"+localStorage.getItem("userName"),
     // headers: {
      //  Authorization: "Bearer "+localStorage.getItem("token"),
...
export const checkProfileSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTweet: (state, action) => {
        console.log('--------deleyted----',action.payload);
